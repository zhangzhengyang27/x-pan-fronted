...
import {
  ...
  Share2, Trash2, PanelLeftClose, PanelLeftOpen, GripVertical, Download as DownloadIcon,
} from '@lucide/vue'
          ...
          <div class="h-full flex flex-col py-3 gap-4">
            ...
            >
              <button
                ...
                :key="item.key"
                type="button"
                :class="['group flex items-center gap-3 mx-2 px-3 h-10 rounded-lg text-sm transition-colors',
                  'hover:bg-[var(--color-surface-2)]',
                  active === item.key ? 'bg-[var(--color-primary-50)] dark:bg-[var(--color-primary-900)]/30 text-[var(--color-primary-600)] font-medium' : '']"
                @click="navigate(item)"
              >
            ...
            <div v-if="!collapsed" class="mt-auto mx-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)] p-3">
              ...
              <div class="h-1.5 overflow-hidden rounded-full bg-[var(--color-surface)]">
                <div class="h-full rounded-full" :class="usedPercent >= 90 ? 'bg-[var(--color-danger)]' : usedPercent >= 70 ? 'bg-[var(--color-warning)]' : 'bg-[var(--color-primary)]'" :style="{width: Math.min(100, usedPercent) + '%'}"/>
              </div>
