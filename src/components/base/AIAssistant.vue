...
import {Sparkles, Send, Bot, User as UserIcon, Loader2, FileText, Image as ImageIcon, Search as SearchIcon} from '@lucide/vue'
...
const messages = ref([
  {role: 'assistant', content: '你好，我是 X-Pan AI 助手 ✨\n\n我可以帮你：\n• 智能搜索文件（"找上周的图片"）\n• 总结 PDF/文档\n• 整理文件夹\n\n试试问我点什么吧～'},
])
const input = ref('')
      ...
      <div ref="scrollRef" class="flex-1 overflow-y-auto pr-1 space-y-3">
        ...
        >
          <div
            ...
            :class="m.role === 'user' ? 'bg-[var(--color-primary-500)] text-white' : 'bg-gradient-to-br from-amber-400 to-pink-500 text-white'"
          >
          ...
          <div
            ...
            :class="m.role === 'user' ? 'bg-[var(--color-primary-500)] text-white rounded-tr-sm' : 'bg-[var(--color-surface-2)] rounded-tl-sm'"
          >
            {{ m.content }}
          </div>
        </div>
        <div v-if="loading" class="flex items-start gap-2">
          <div class="size-8 shrink-0 rounded-full bg-gradient-to-br from-amber-400 to-pink-500 text-white flex items-center justify-center">
            <Sparkles :size="14"/>
          </div>
