...
function toggleAll() {
  ...
  else emit('update:selected', props.data.map((r, i) => getRowKey(r, i)))
}
...
<template>
  <div class="w-full overflow-x-auto rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)]">
    <table class="w-full text-sm border-collapse">
        ...
        <tr class="border-b border-[var(--color-border)]">
          <th v-if="selectable" class="w-12 px-4 py-3 text-left">
            <button
              ...
              :class="(allSelected || partialSelected) && 'bg-[var(--color-primary-600)] border-[var(--color-primary-600)] text-white'"
      ...
      <tbody v-else-if="error">
        <tr>
          <td :colspan="columns.length + (selectable ? 1 : 0)" class="px-4 py-16 text-center text-[var(--color-danger)]">
        ...
        >
          <td v-if="selectable" class="w-12 px-4 py-3" @click.stop>
            <button
              type="button"
              class="size-4 rounded border border-[var(--color-border-strong)] flex items-center justify-center transition-colors"
              :class="isSelected(row) && 'bg-[var(--color-primary-600)] border-[var(--color-primary-600)] text-white'"
