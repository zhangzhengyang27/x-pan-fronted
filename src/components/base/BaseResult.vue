...
<template>
  <div class="flex flex-col items-center text-center py-12 px-6">
    <component :is="iconMap[status]" :size="72" :stroke-width="1.5" :class="colorMap[status]" class="mb-4"/>
    <h1 class="text-2xl font-semibold m-0 mb-2">{{ title }}</h1>
    <p v-if="description" class="text-sm text-[var(--color-text-muted)] m-0 max-w-md">{{ description }}</p>
