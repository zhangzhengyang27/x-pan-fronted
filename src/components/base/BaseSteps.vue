      ...
      <div class="flex items-center gap-2 shrink-0">
        <span
          :class="cn(
            ...
            i === active && 'bg-[var(--color-primary-600)] text-white ring-4 ring-[var(--color-primary-100)]',
            i > active && 'bg-[var(--color-surface-2)] text-[var(--color-text-muted)] border border-[var(--color-border)]',
          )"
        >
        ...
        <div class="flex flex-col leading-tight">
          <span
            :class="cn(
              'text-sm',
              i <= active ? 'text-[var(--color-text)] font-medium' : 'text-[var(--color-text-muted)]',
            )"
          >{{ s.title }}</span>
          <span v-if="s.description" class="text-xs text-[var(--color-text-muted)]">{{ s.description }}</span>
        </div>
      </div>
      <span
