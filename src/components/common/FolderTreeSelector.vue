...
<template>
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="close">
    <div class="w-[460px] max-w-[92vw] bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] shadow-xl overflow-hidden">
      <header class="h-12 px-4 flex items-center justify-between border-b border-[var(--color-border)]">
        <h3 class="text-sm font-semibold text-[var(--color-text)]">{{ title }}</h3>
        <button type="button" class="size-7 rounded-md hover:bg-[var(--color-surface-2)] flex items-center justify-center text-[var(--color-text-muted)]" @click="close">
      ...
      <div class="p-2 h-72 overflow-auto">
        <div v-if="loading" class="flex h-full items-center justify-center gap-2 text-sm text-[var(--color-text-muted)]">
          <Loader2 :size="16" class="animate-spin"/>
          加载中...
        </div>
        <div v-else-if="error" class="flex h-full flex-col items-center justify-center gap-2 text-sm text-[var(--color-danger)]">
          <AlertCircle :size="20"/>
          {{ error }}
          <BaseButton variant="ghost" size="sm" @click="loadTree">重试</BaseButton>
        </div>
        <div v-else-if="!hasTree" class="flex h-full items-center justify-center text-sm text-[var(--color-text-muted)]">
          暂无可用文件夹
        </div>
        <ul v-else class="m-0 p-0 list-none">
          <li>
            <button
              ...
              :class="selectedId === null ? 'bg-[var(--color-primary-50)] text-[var(--color-primary-700)] dark:bg-[var(--color-primary-900)]/30 dark:text-[var(--color-primary-300)]' : 'hover:bg-[var(--color-surface-2)] text-[var(--color-text)]'"
              @click="pickRoot"
            >
      ...
      <footer class="h-12 px-4 flex items-center justify-end gap-2 border-t border-[var(--color-border)] bg-[var(--color-surface-2)]">
