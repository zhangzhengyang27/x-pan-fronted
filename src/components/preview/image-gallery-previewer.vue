...
<template>
  ...
  <div
    ...
    class="fixed inset-0 bg-black text-white flex flex-col z-50"
  >
    <header class="h-14 px-5 flex items-center justify-between bg-black/60 backdrop-blur border-b border-white/5 z-10">
      ...
      <div class="flex items-center gap-1">
        ...
        <button type="button" class="size-9 rounded-lg hover:bg-white/10 flex items-center justify-center" aria-label="下载" @click="download">
          <Download :size="16"/>
        </button>
    ...
    <main class="flex-1 flex items-center justify-center relative overflow-hidden" @wheel="onWheel">
      <button v-if="items.length > 1" type="button" class="absolute left-4 top-1/2 -translate-y-1/2 z-10 size-12 rounded-full bg-black/40 hover:bg-black/60 backdrop-blur flex items-center justify-center transition-colors" aria-label="上一张" @click="prev">
        <ChevronLeft :size="22"/>
      </button>
      <button v-if="items.length > 1" type="button" class="absolute right-4 top-1/2 -translate-y-1/2 z-10 size-12 rounded-full bg-black/40 hover:bg-black/60 backdrop-blur flex items-center justify-center transition-colors" aria-label="下一张" @click="next">
    ...
    <footer v-if="items.length > 1" class="h-24 px-5 flex items-center gap-2 overflow-x-auto border-t border-white/5 bg-black/60 backdrop-blur">
      <button
        v-for="(it, i) in items"
        :key="it.fileId"
        type="button"
        class="shrink-0 h-20 w-20 rounded-lg overflow-hidden border-2 transition-all"
        :class="i === activeIndex ? 'border-[var(--color-primary-400)] opacity-100 scale-105' : 'border-transparent opacity-60 hover:opacity-100'"
        :aria-label="it.name"
        @click="pick(i)"
      >
        <img v-if="urlMap[it.fileId]" :src="urlMap[it.fileId]" :alt="it.name" class="w-full h-full object-cover"/>
  ...
  <div v-else class="relative w-full h-full flex items-center justify-center bg-[var(--color-surface-2)]">
    <button v-if="items.length > 1" type="button" class="absolute left-3 top-1/2 -translate-y-1/2 z-10 size-10 rounded-full bg-black/40 hover:bg-black/60 backdrop-blur text-white flex items-center justify-center transition-colors" aria-label="上一张" @click="prev">
      <ChevronLeft :size="20"/>
    </button>
    <button v-if="items.length > 1" type="button" class="absolute right-3 top-1/2 -translate-y-1/2 z-10 size-10 rounded-full bg-black/40 hover:bg-black/60 backdrop-blur text-white flex items-center justify-center transition-colors" aria-label="下一张" @click="next">
      <ChevronRight :size="20"/>
    </button>
    <img v-if="currentSrc" :src="currentSrc" :alt="currentItem?.name" class="max-h-full max-w-full object-contain"/>
