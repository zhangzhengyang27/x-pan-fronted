...
<template>
  <div class="min-h-screen flex items-center justify-center relative overflow-hidden bg-[var(--color-bg)] p-gutter">
    <!-- 环境光晕 -->
    <div
    ...
    <div class="relative z-10 w-full max-w-2xl">
      ...
      <div class="flex items-center gap-2 mb-6">
        ...
        <span class="text-sm text-[var(--color-text-muted)]">· 协议中心</span>
      </div>
      ...
      >
        ...
        <p class="text-sm mt-2 text-[var(--color-text-muted)]">{{ meta.subtitle }}</p>
        <p class="text-xs mt-1 text-[var(--color-text-muted)]">{{ meta.updated }}</p>

        <div class="h-px my-6" style="background-color: var(--color-border);" />

        <!-- 协议正文 -->
        <article class="space-y-5 text-sm leading-relaxed text-[var(--color-text)]">
          <template v-if="!isPrivacy">
            <section>
              <h2 class="text-base font-semibold mb-2 text-[var(--color-text)]">一、服务说明</h2>
              <p class="text-[var(--color-text-muted)]">
                X Pan 为您提供个人文件的存储、管理与分享服务。您在使用本服务前，应当仔细阅读并充分理解本协议的全部内容，尤其是以加粗形式提示的责任豁免或限制条款。
              </p>
            </section>
            <section>
              <h2 class="text-base font-semibold mb-2 text-[var(--color-text)]">二、账户注册与安全</h2>
              <p class="text-[var(--color-text-muted)]">
                您需对账户下的所有活动负责。请妥善保管登录凭证，因密码泄露、设备丢失等原因导致的损失，由您自行承担。我们不会对任何非因平台原因造成的损失承担责任。
              </p>
            </section>
            <section>
              <h2 class="text-base font-semibold mb-2 text-[var(--color-text)]">三、用户行为规范</h2>
              <p class="text-[var(--color-text-muted)]">
                您不得利用本服务上传、存储或传播任何违法违规、侵权或危害网络安全的内容。我们有权对违规内容予以删除，并视情节暂停或终止您的账户。
              </p>
            </section>
            <section>
              <h2 class="text-base font-semibold mb-2 text-[var(--color-text)]">四、存储与可用性</h2>
              <p class="text-[var(--color-text-muted)]">
                我们致力于保障服务的高可用与数据可靠，但不对不可抗力、网络故障或维护导致的临时不可用承担责任。重要数据请您自行做好备份。
              </p>
            </section>
            <section>
              <h2 class="text-base font-semibold mb-2 text-[var(--color-text)]">五、协议的变更</h2>
              <p class="text-[var(--color-text-muted)]">
          ...
          <template v-else>
            <section>
              <h2 class="text-base font-semibold mb-2 text-[var(--color-text)]">一、我们收集的信息</h2>
              <p class="text-[var(--color-text-muted)]">
                为提供存储与登录能力，我们仅收集您注册时提供的账户名、邮箱及登录设备的基础信息，不会收集与服务无关的敏感个人信息。
              </p>
            </section>
            <section>
              <h2 class="text-base font-semibold mb-2 text-[var(--color-text)]">二、信息的使用</h2>
              <p class="text-[var(--color-text-muted)]">
                您的个人信息仅用于身份验证、安全防护与服务通知。我们不会向第三方出售或提供您的个人信息，法律法规另有规定的除外。
              </p>
            </section>
            <section>
              <h2 class="text-base font-semibold mb-2 text-[var(--color-text)]">三、数据存储与安全</h2>
              <p class="text-[var(--color-text-muted)]">
                您的文件内容采用隔离存储，并通过传输加密与访问控制保护。我们采取合理的技术措施防范未授权访问，但无法保证绝对安全。
              </p>
            </section>
            <section>
              <h2 class="text-base font-semibold mb-2 text-[var(--color-text)]">四、您的权利</h2>
              <p class="text-[var(--color-text-muted)]">
                您有权随时查询、更正或删除您的个人信息，并可在注销账户后要求我们删除相关数据。如对本政策有疑问，可通过站内反馈联系我们。
              </p>
            </section>
            <section>
              <h2 class="text-base font-semibold mb-2 text-[var(--color-text)]">五、政策的变更</h2>
              <p class="text-[var(--color-text-muted)]">
      ...
      <p class="mt-8 text-center text-xs font-mono text-[var(--color-text-muted)]">
