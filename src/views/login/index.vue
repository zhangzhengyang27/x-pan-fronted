...
const { setParentId, setDefaultParentId, setDefaultParentFilename, clearParentCache } = fileStore
    ...
    <div class="w-full max-w-[380px] mx-4 relative z-10">
      ...
      <div class="quark-card p-7">
        <form class="space-y-5" @submit.prevent="doLogin">
          ...
          <div>
            <div class="flex items-center justify-between mb-2">
              <label class="block text-sm font-medium text-(--color-text)">密码</label>
              <button
                type="button"
                class="text-xs transition-colors text-primary-500 hover:text-primary-600"
                @click="goForget"
              >
                忘记密码？
              </button>
        ...
        <div class="mt-5 flex items-center gap-3">
          ...
          <span class="text-xs text-(--color-text-muted)">还没有账号？</span>
          <div class="flex-1 h-px bg-(--color-border)" />
        </div>
