...
function doReset() {
  ...
  if (form.password.length < 8 || form.password.length > 16) return ElMessage.error('密码为 8-16 位')
  ...
  <div class="min-h-screen flex items-center justify-center bg-[var(--color-bg)] p-6">
    <div class="w-full max-w-sm">
      <div class="text-center mb-8">
        <div class="size-14 mx-auto rounded-2xl bg-[var(--color-primary-50)] dark:bg-[var(--color-primary-900)]/30 flex items-center justify-center mb-4 text-[var(--color-primary-600)]">
          <ShieldCheck :size="28"/>
        </div>
      ...
      <form class="flex flex-col gap-4" @submit.prevent="doReset">
        ...
        <BaseField label="新密码" required>
          <BaseInput v-model="form.password" type="password" show-password placeholder="8-16 位" :prefix="KeyRound"/>
        </BaseField>
        <BaseField label="确认密码" required>
          <BaseInput v-model="form.rePassword" type="password" show-password placeholder="再次输入" :prefix="KeyRound"/>
        </BaseField>

        <BaseButton variant="primary" size="lg" :loading="loading" block @click="doReset">重置密码</BaseButton>
      </form>
